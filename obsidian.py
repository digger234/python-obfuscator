...
def __spell__(text, seed):
   ...
   return out
def __smelter__(n, seed):
   fog = hashlib.sha256(seed).digest(); big = int.from_bytes(fog[0:8], 'little') + (1 << 63); off = int.from_bytes(fog[8:16], 'little'); mult = int.from_bytes(fog[16:24], 'little') | 1; key = int.from_bytes(fog[24:32], 'little')
   core = ((n + big + off) * mult) ^ key
   return '((((%d ^ %d) // %d) - %d) - %d)' % (core, key, mult, off, big)
def __flare__(blob):
   glow = 0
   at = 0
   while at < len(blob):
...
def __mint__(used, seed, mint):
   ...
   while True:
        fog = hashlib.sha256(seed + mint[0].to_bytes(4, 'little')).digest(); wide = 1 + (fog[0] & 1); rows = ['__']; slot = 0; at = 1
        while slot < wide:
            if at + 2 >= len(fog):
        ...
        if norm != name or name in used or norm in used or name == __totem__: continue
...
def __sigil__(seed, count, used=None):
   ...
   return rows
__totem__ = __rune__(set(), b'obsidianward', b'ward')
__sexgay__.add(__totem__)
def __crown__(raw):
   glow = 0
   bend = 0
...
def __prism__(name, data):
    ...
    uni=''.join(one for one, __meow__ in __marble__(name.encode('utf-8','surrogatepass')+b'prismuni',2)).encode('utf-8','surrogatepass');a,b,c,d=__sigil__(name.encode('utf-8','surrogatepass')+b'prism'+uni,4)
...
def __iris__(name, way):
    uni=''.join(one for one, __meow__ in __marble__(name.encode('utf-8','surrogatepass')+bytes([way])+b'irisuni',2)).encode('utf-8','surrogatepass');tail = ".decode('utf-8')" if way else ""; a, b, c, d, e, f, g, h, i, j = __sigil__(name.encode('utf-8', 'surrogatepass') + bytes([way]) + b'iris' + uni, 10)
    mod=__spell__('functools', name.encode('utf-8','surrogatepass')+bytes([way])+b'irismod');red=__spell__('reduce', name.encode('utf-8','surrogatepass')+bytes([way])+b'irisred');hexn=__spell__('fromhex', name.encode('utf-8','surrogatepass')+bytes([way])+b'irishex')
    return f"{name}=lambda {a},{b},{c},{d}:(lambda {e}:(lambda {f}:((len({e}),sum({e})&0xffffffff,{e}[:1],{e}[-1:],{f}[0],{f}[1])!={d} and 1/0,{e}{tail})[-1])(getattr({__aim__()}({mod}),{red})(lambda {g},{h}:(({g}[0]+{h}[1]+{h}[0])&0xffffffff,((({g}[1]^((({h}[1]+1)*({h}[0]+3))&0xffffffff))<<5)|(({g}[1]^((({h}[1]+1)*({h}[0]+3))&0xffffffff))>>27))&0xffffffff),enumerate({e}),(0,0))))((lambda {f}:bytes(({i}^{b}^{f}[{j}]) for {j},{i} in enumerate({a})))(getattr(bytes,{hexn})({c})))"
def __plume__(name):
   way = 1
...
def __dump__(code):
   ...
   if row is None:
         ...
         if len(__dumpbag__) > 65536: __dumpbag__.clear()
   return row
def __raw__(code):
   ...
   return row
def __purge__():
   for bag in (__dripbag__,__dumpbag__,__tuffbag__,__shapebag__,__walkbag__,__gaskbag__,__rawbag__,__soulbag__):
        try: bag.clear()
        except: pass
   try:
        gc = __import__('gc')
...
def __drip__(code):
   ...
   if len(__dripbag__) > 65536: __dripbag__.clear()
   return rows
def __scrub__(code, seed):
   ...
   def one(obj, deep, env=None):
        ...
        turn[0]+=1
        raw=hashlib.sha256(seed+deep.to_bytes(4,'little')+turn[0].to_bytes(4,'little')+obj.co_code+len(obj.co_consts).to_bytes(4,'little')+len(obj.co_names).to_bytes(4,'little')).digest()
...
def __saltcode__(code, seed):
   kind=type(code); seen={}; used=set(); turn=[0]
   def one(obj, deep):
        mark=id(obj)
        if mark in seen: return seen[mark]
        turn[0]+=1
        raw=hashlib.blake2b(seed+deep.to_bytes(4,'little')+turn[0].to_bytes(4,'little')+obj.co_code+len(obj.co_consts).to_bytes(4,'little'),digest_size=32).digest()
...
def __soul__(code):
   num=0;sha=hashlib.sha256();one=hashlib.sha1()
   for obj in __drip__(code):
        for row in obj.co_names+obj.co_varnames:
             raw=row.encode('utf-8');sha.update(raw);one.update(raw);num+=1
   return (num, sha.hexdigest(), one.hexdigest())
def __wisp__(code):
   num=0;sha=hashlib.sha256();one=hashlib.sha1()
   for obj in __drip__(code):
        raw=obj.co_name.encode('utf-8')+obj.co_filename.encode('utf-8')+obj.co_argcount.to_bytes(max(1,(obj.co_argcount.bit_length()+8)//8),'little',signed=True)+obj.co_firstlineno.to_bytes(max(1,(obj.co_firstlineno.bit_length()+8)//8),'little',signed=True)+len(obj.co_consts).to_bytes(max(1,(len(obj.co_consts).bit_length()+8)//8),'little',signed=True);sha.update(raw);one.update(raw);num+=1
   return (num, sha.hexdigest(), one.hexdigest())
def __gasket__(blob):
    hit = __gaskbag__.get(blob)
    if hit is not None: return hit
    out = bytes([0]) + zlib.compress(blob, 9)
    if len(__gaskbag__) > 65536: __gaskbag__.clear()
    __gaskbag__[blob] = out
    return out
def __weld__(blob, key):
    glow = key & 255; drift = ((key >> 8) & 255) or 73; tint = ((key >> 16) & 255) or 19
    ks = bytearray(512)
    for slot in range(512):
        glow = (glow + drift + slot + tint) & 255; ks[slot] = glow ^ ((tint + slot) & 255)
    n = len(blob)
    if n <= 512:
        return bytes(one ^ ks[slot] for slot, one in enumerate(blob))
    ks = bytes(ks)
    full = ks * (n // 512) + ks[:n % 512]
    return (int.from_bytes(blob, 'little') ^ int.from_bytes(full, 'little')).to_bytes(n, 'little')
...
def __nexus__(raw, seed, plan, face, loom):
    ...
    bag.extend(hashlib.sha512(zlib.compress(core, 9) + seed).digest())
    hist = bytearray(256)
    ...
    pack = zlib.compress(bytes(bag), 9); mark = marshal.dumps((len(src), len(zlib.compress(core, 9)), zlib.crc32(pack) & 0xffffffff, hashlib.sha256(pack).digest()))
...
def __mote__(seed, used, slot):
    ...
    c = a ^ b
    return f"(lambda {left},{right},{mark}:({left}^{right})=={mark} or (__meow__ for __meow__ in ()).throw(SystemExit))({a},{b},{c})"
...
def __hist__(rows):
    ...
    step = 1; n = len(rows)
    if n > 2048: step = (n + 2047) // 2048
    if step > 1: rows = rows[::step]
    bag = {}
    for one in rows:
...
def __shape__(tree):
    ...
    return row
def __walk__(tree):
    mark=id(tree);hit=__walkbag__.get(mark)
    if hit is not None:return hit
    bag = []
    edge = []
    deep = []
    ...
    out=(tuple(bag), tuple(edge), tuple(deep), wide);__walkbag__[mark]=out
    if len(__walkbag__) > 128: __walkbag__.clear()
    return out
def __shapeid__(tree, seed):
    bag, edge, deep, wide = __walk__(tree)
...
def __literal__(tree, seed):
    ...
    raw = []
    flags = [0, 0, 0, 0]
    for node in __sweep__(tree, (ast.Constant,)):
...
def __label__(tree, seed):
    ...
    attr = []
    call = []
    for node in __sweep__(tree, (ast.Attribute, ast.Call, ast.Load, ast.Name, ast.Store)):
...
def __scope__(tree, seed):
    ...
    args = []
    deco = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef)):
...
def __set__(tree, seed):
    bag = []
    dims = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.Assign, ast.AugAssign, ast.NamedExpr)):
...
def __pat__(tree, seed):
    bag = []
    vals = []
    for node in __sweep__(tree, (ast.Match, ast.MatchAs, ast.MatchClass, ast.MatchMapping, ast.MatchOr, ast.MatchSequence, ast.MatchSingleton, ast.MatchStar, ast.MatchValue)):
...
def __loop__(tree, seed):
    bag = []
    marks = []
    for node in __sweep__(tree, (ast.AsyncFor, ast.Break, ast.Continue, ast.For, ast.While)):
...
def __comp__(tree, seed):
    bag = []
    gens = []
    for node in __sweep__(tree, (ast.DictComp, ast.GeneratorExp, ast.ListComp, ast.SetComp)):
...
def __veinsow__(tree):
    mark = id(tree); row = __soulbag__.get(mark)
    if row is None:
        levels = {}
        lrows = []
        pairs = []
        ...
        hold = [(tree, 0, '')]
        while hold:
            node, deep, last = hold.pop()
            now = type(node).__name__
            cc = 0
            for one in ast.iter_child_nodes(node):
                cc += 1
                hold.append((one, deep + 1, now))
            levels[deep] = levels.get(deep, 0) + 1
            lrows.append((deep, now, cc))
            if last:
                pairs.append((last, now))
            total += 1
            if deep > maxd: maxd = deep
        row = (levels, __hist__(lrows), __hist__(pairs), len(pairs), total, maxd)
        __soulbag__[mark] = row
        if len(__soulbag__) > 64: __soulbag__.clear()
    return row
def __ref__(tree, seed):
    _, _, phist, paircount, _, _ = __veinsow__(tree)
    fog = __mix__(seed, (phist, paircount))
    return (paircount, phist, fog.hex())
def __depth__(tree, seed):
    levels, _, _, _, _, _ = __veinsow__(tree)
    rows = tuple(sorted(levels.items()))
    fog = __mix__(seed, rows)
    return (len(rows), rows, fog.hex())
def __tile__(tree, seed):
    bag = []
    rows = list(__sweep__(tree, 'all'))
...
def __context__(tree, seed):
    bag = []
    for node in __sweep__(tree, 'all'):
...
def __discard__(tree, seed):
    bag = []
    for node in __sweep__(tree, (ast.Assert, ast.Break, ast.Continue, ast.Delete, ast.Expr, ast.Pass)):
...
def __wth__(tree, seed):
    bag = []
    for node in __sweep__(tree, (ast.AsyncWith, ast.With)):
...
def __glyph__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.Attribute, ast.ClassDef, ast.FunctionDef, ast.Name, ast.alias, ast.arg)):
...
def __gram__(tree, seed):
    rows = [type(one).__name__ for one in __sweep__(tree, 'all')]
...
def __tok__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Name, ast.alias, ast.arg)):
...
def __bop__(tree, seed):
    rows = []; bin = []; una = []
    for node in __sweep__(tree, (ast.BinOp, ast.BoolOp, ast.UnaryOp)):
...
def __cmpr__(tree, seed):
    rows = []; lens = []
    for node in __sweep__(tree, (ast.Compare,)):
...
def __dial__(tree, seed):
    rows = []; argc = []
    for node in __sweep__(tree, (ast.Attribute, ast.Call, ast.Name)):
...
def __river__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFor, ast.For, ast.If, ast.IfExp, ast.While)):
...
def __coil__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.DictComp, ast.GeneratorExp, ast.ListComp, ast.SetComp, ast.comprehension)):
...
def __fmt__(tree, seed):
    rows = []; bag = []; spec = []
    for node in __sweep__(tree, (ast.FormattedValue, ast.JoinedStr)):
...
def __match__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Match, ast.MatchAs, ast.MatchClass, ast.MatchMapping, ast.MatchOr, ast.MatchSequence, ast.MatchSingleton, ast.MatchStar, ast.MatchValue, ast.match_case)):
...
def __imp__(tree, seed):
    rows = []; bag = []; als = []; lvl = []
    for node in __sweep__(tree, (ast.Import, ast.ImportFrom)):
...
def __func__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.FunctionDef)):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
...
def __arg__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.FunctionDef, ast.Lambda)):
...
def __clan__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.ClassDef,)):
...
def __anno__(tree, seed):
    rows = []; raw = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.AsyncFunctionDef, ast.FunctionDef, ast.arg)):
...
def __out__(tree, seed):
    rows = []; raw = []
    for node in __sweep__(tree, (ast.Await, ast.Return, ast.Yield, ast.YieldFrom)):
...
def __slice__(tree, seed):
    rows = []; dim = []
    for node in __sweep__(tree, (ast.Slice, ast.Starred, ast.Subscript)):
...
def __seq__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Dict, ast.List, ast.Set, ast.Tuple)):
...
def __paper__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef, ast.Module)):
...
def __tree__(tree, seed):
    ...
    while hold:
        ...
        child = tuple(ast.iter_child_nodes(node))
        cc = len(child)
        if len(rows) < 4096:
            rows.append((typ, depth, field, cc))
        total += 1
        if depth > maxd: maxd = depth
        for one in reversed(child): hold.append((one, depth + 1, typ))
    fog = __mix__(seed, (tuple(rows), total, maxd))
    return (total, fog.hex())
...
def __attr__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Attribute,)):
...
def __kwd__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.keyword,)):
...
def __place__(tree, seed):
    rows = []
    stack = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef, ast.Lambda)):
...
def __sym__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Global, ast.Name, ast.Nonlocal)):
...
def __blend__(tree, seed):
    ...
    for node in ast.walk(tree):
        cc = 0
        for _ in ast.iter_child_nodes(node): cc += 1
        rows.append((type(node).__name__, cc, getattr(node, 'lineno', 0), getattr(node, 'col_offset', 0)))
...
def __pak__(code, seed):
    rows = []
    for one in __drip__(code)[::16]:
        raw = __dump__(one)
        pack = zlib.compress(raw, 9)
...
def __flat__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.Constant, ast.FunctionDef, ast.Import, ast.ImportFrom)):
        if isinstance(node, ast.Constant):
            val = node.value
...
def __prong__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Import, ast.ImportFrom, ast.Name)):
        if isinstance(node, ast.Name):
...
def __level__(tree, seed):
    _, lhist, _, _, total, maxd = __veinsow__(tree)
    fog = __mix__(seed, (lhist, maxd, total))
    return (total, fog.hex())
...
def __crestattr__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Name)):
        if isinstance(node, ast.Attribute):
            cur = node
            chain = []
            while isinstance(cur, ast.Attribute):
...
def __crestname__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef, ast.Name, ast.alias, ast.arg)):
...
def __crestop__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.BinOp, ast.BoolOp, ast.Compare, ast.UnaryOp)):
...
def __crestapi__(tree, seed):
    ...
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Constant, ast.Name)):
...
def __crestnet__(tree, seed):
    ...
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.keyword)):
...
def __crestasync__(tree, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFor, ast.AsyncFunctionDef, ast.AsyncWith, ast.Attribute, ast.Await, ast.Call)):
...
def __kiv__(tree, code, seed):
    ...
    for node in __sweep__(tree, (ast.Import, ast.ImportFrom, ast.Call)):
        if isinstance(node, ast.Import):
...
def __rice__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.FunctionDef, ast.Lambda, ast.Return, ast.Yield)):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.Lambda)):
...
def __sorn__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncWith, ast.ClassDef, ast.With)):
...
def __vor__(tree, code, seed):
    ...
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Name, ast.keyword)):
...
def __wool__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Dict, ast.List, ast.Set, ast.Starred, ast.Tuple)):
...
def __yul__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AugAssign, ast.Delete, ast.Slice, ast.Subscript)):
...
def __ziv__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.BinOp, ast.BoolOp, ast.Compare, ast.UnaryOp, ast.AugAssign, ast.IfExp)):
        if isinstance(node, ast.BinOp): rows.append(('bin', type(node.op).__name__, type(node.left).__name__, type(node.right).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.BoolOp): rows.append(('bool', type(node.op).__name__, len(node.values), tuple(type(one).__name__ for one in node.values[:16]), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.UnaryOp): rows.append(('unary', type(node.op).__name__, type(node.operand).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Compare): rows.append(('cmp', type(node.left).__name__, tuple(type(one).__name__ for one in node.ops), len(node.comparators), getattr(node, 'lineno', 0)))
...
def __lum__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.FormattedValue, ast.JoinedStr, ast.NamedExpr)):
...
def __orz__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Delete, ast.Global, ast.Nonlocal, ast.alias, ast.arg)):
        if isinstance(node, ast.Delete): rows.append(('del', tuple(type(one).__name__ for one in node.targets[:16]), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Global): rows.append(('global', tuple(node.names), getattr(node, 'lineno', 0)))
...
def __vyn__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.Assert, ast.Assign, ast.AugAssign, ast.Break, ast.Continue, ast.Expr, ast.Pass)):
...
def __lime__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFor, ast.AsyncFunctionDef, ast.AsyncWith, ast.Attribute, ast.Await, ast.Call, ast.Name)):
        if isinstance(node, ast.Await): rows.append(('await', type(node.value).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.AsyncFor): rows.append(('asyncfor', type(node.target).__name__, type(node.iter).__name__, len(node.body), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.AsyncWith): rows.append(('asyncwith', len(node.items), len(node.body), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.AsyncFunctionDef): rows.append(('asyncfn', getattr(node, 'name', ''), len(getattr(node, 'body', [])), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Attribute): rows.append(('attr', node.attr, type(node.value).__name__, type(node.ctx).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Name): rows.append(('name', node.id, type(node.ctx).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Call):
...
def __nuv__(tree, code, seed):
    ...
    marks = tuple((type(node).__name__, getattr(node, 'lineno', 0)) for node in __sweep__(tree, (ast.Constant, ast.FormattedValue, ast.JoinedStr)) if isinstance(node, (ast.Constant, ast.JoinedStr, ast.FormattedValue)))[:1024]
...
def __piv__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.Assign, ast.Delete, ast.NamedExpr)):
...
def __topaz__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Constant, ast.FormattedValue, ast.JoinedStr)):
        if isinstance(node, ast.JoinedStr): rows.append(('join', len(node.values), tuple(type(one).__name__ for one in node.values), getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.FormattedValue): rows.append(('fmt', node.conversion, type(node.value).__name__, type(node.format_spec).__name__ if node.format_spec else '', getattr(node, 'lineno', 0)))
...
def __tov__(tree, code, seed):
    ...
    for node in __sweep__(tree, (ast.Slice, ast.Starred, ast.Subscript)):
        if isinstance(node, ast.Subscript): rows.append(('sub', type(node.value).__name__, type(node.slice).__name__, type(node.ctx).__name__, getattr(node, 'lineno', 0)))
        elif isinstance(node, ast.Slice): rows.append(('slice', type(node.lower).__name__ if node.lower else '', type(node.upper).__name__ if node.upper else '', type(node.step).__name__ if node.step else '', getattr(node, 'lineno', 0)))
...
def __vok__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncWith, ast.With, ast.withitem)):
...
def __kao__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Break, ast.Continue, ast.If, ast.IfExp, ast.Pass)):
...
def __neo__(tree, code, seed):
    ...
    for node in __sweep__(tree, (ast.Attribute, ast.Constant)):
        if isinstance(node, ast.Attribute) and node.attr in keys: rows.append(('attr', node.attr, type(node.value).__name__, getattr(node, 'lineno', 0)))
...
def __uio__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Call, ast.keyword)):
        if isinstance(node, ast.keyword): rows.append((node.arg or '*', type(node.value).__name__, getattr(node, 'lineno', 0)))
...
def __vio__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Expr, ast.Return, ast.Yield, ast.YieldFrom)):
...
def __zio__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.arg)):
...
def __kra__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Assert, ast.Raise)):
...
def __lra__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.For, ast.While)):
...
def __pra__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Lambda, ast.NamedExpr)):
...
def __qra__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.ClassDef, ast.keyword)):
...
def __rra__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Expr, ast.Module)):
...
def __mosaic__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.MatchClass, ast.MatchMapping, ast.MatchSequence)):
...
def __lamb__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Lambda,)):
...
def __warden__(tree, code, seed):
    ...
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef, ast.Lambda)):
        ...
        if isinstance(node, ast.Lambda):
            args = node.args
            rows.append(('lambda', len(args.posonlyargs), len(args.args), len(args.kwonlyargs), int(args.vararg is not None), int(args.kwarg is not None), type(node.body).__name__, getattr(node, 'lineno', 0)))
    raw = []
    for one in __drip__(code)[:256]:
...
def __allay__(tree, code, seed):
    rows = []
    stack = []
    for node in __sweep__(tree, (ast.Call, ast.Import, ast.ImportFrom, ast.Name)):
        if isinstance(node, ast.Import):
            for alias in node.names:
...
def __hoglin__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.FormattedValue, ast.JoinedStr, ast.Slice, ast.Starred, ast.Subscript)):
        if isinstance(node, ast.JoinedStr):
...
def __panda__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AnnAssign, ast.Assign, ast.AugAssign, ast.Global, ast.NamedExpr, ast.Nonlocal)):
        if isinstance(node, ast.Assign):
...
def __llama__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Assert, ast.Await, ast.Raise, ast.Return, ast.Yield, ast.YieldFrom)):
        if isinstance(node, ast.Return):
...
def __ocelot__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Break, ast.Continue, ast.Delete, ast.Expr, ast.Pass)):
        if isinstance(node, ast.Delete):
...
def __ravager__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Dict, ast.List, ast.Set, ast.Tuple, ast.keyword)):
...
def __phantom__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFor, ast.AsyncFunctionDef, ast.AsyncWith, ast.ExceptHandler, ast.TryStar)):
...
def __fox__(tree, code, seed):
    ...
    for node in __sweep__(tree, (ast.Constant, ast.Name)):
        if isinstance(node, ast.Name):
            low = node.id.lower()
...
def __parrot__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.MatchClass, ast.MatchMapping, ast.MatchOr, ast.MatchSequence, ast.pattern)):
...
def __rabbit__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.Constant, ast.Expr, ast.FunctionDef, ast.Module)):
...
def __salmon__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.Attribute, ast.Call, ast.Subscript)):
        if isinstance(node, ast.Attribute):
...
def __piglin__(tree, code, seed):
    rows = []
    bank = {}
    for node in __sweep__(tree, (ast.Call, ast.Constant, ast.Load, ast.Name, ast.Store)):
        if isinstance(node, ast.Constant):
...
def __ghast__(tree, code, seed):
    rows = []
    for node in __sweep__(tree, (ast.AsyncFunctionDef, ast.ClassDef, ast.FunctionDef, ast.Name)):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            names = []
...
def __sweep__(tree, spec):
    key = id(tree)
    hoard = __beacon__.get(key)
    if hoard is None:
        hoard = __blaze__(tree)
        __beacon__[key] = hoard
        if len(__beacon__) > 64:
            __beacon__.clear()
    if spec == 'all':
        return iter(hoard['all'])
    wanted = []
    for t in spec:
        lst = hoard.get(t)
        if lst is None and t.__subclasses__():
            for c in __guild__(t):
                if c in hoard and hoard[c] not in wanted:
                    wanted.append(hoard[c])
            continue
        if lst is not None and lst not in wanted:
            wanted.append(lst)
    if not wanted:
        return iter(())
    step = 1; total = sum(len(lst) for lst in wanted)
    if total > 2048: step = (total + 2047) // 2048
    if len(wanted) == 1:
        base = wanted[0]
        if step > 1:
            return (one[1] for i, one in enumerate(base) if i % step == 0)
        return (one[1] for one in base)
    merged = heapq.merge(*wanted)
    if step > 1:
...
def __chart__(tree, code, stem, path, seed):
    ...
    try:
        ...
        dual = (((b'vex', __vex__), (b'zod', __zod__), (b'kiv', __kiv__), (b'mav', __mav__), (b'night', __night__), (b'pyr', __pyr__), (b'qel', __qel__), (b'rice', __rice__), (b'sorn', __sorn__), (b'tav', __tav__), (b'umber', __umber__), (b'vor', __vor__), (b'wool', __wool__), (b'yul', __yul__), (b'ziv', __ziv__), (b'lum', __lum__), (b'orz', __orz__), (b'dusk', __dusk__), (b'peace', __peace__), (b'quill', __quill__), (b'rust', __rust__), (b'siv', __siv__), (b'tick', __tick__), (b'uvo', __uvo__), (b'vyn', __vyn__)), ((b'wok', __wok__), (b'xul', __xul__), (b'yarn', __yarn__), (b'zok', __zok__), (b'lime', __lime__), (b'cedar', __cedar__), (b'nuv', __nuv__), (b'oxa', __oxa__), (b'piv', __piv__), (b'topaz', __topaz__), (b'tov', __tov__), (b'uzn', __uzn__), (b'vok', __vok__), (b'xir', __xir__), (b'yok', __yok__), (b'ruby', __ruby__)), ((b'kao', __kao__), (b'leo', __leo__), (b'mio', __mio__), (b'neo', __neo__), (b'qio', __qio__), (b'rio', __rio__), (b'sio', __sio__), (b'uio', __uio__), (b'vio', __vio__), (b'wio', __wio__)), ((b'zio', __zio__), (b'kra', __kra__), (b'lra', __lra__), (b'pra', __pra__), (b'qra', __qra__), (b'rra', __rra__)), ((b'mosaic', __mosaic__), (b'lamb', __lamb__)), ((b'bee', __bee__), (b'camel', __camel__), (b'warden', __warden__), (b'allay', __allay__), (b'breeze', __breeze__), (b'strider', __strider__), (b'hoglin', __hoglin__), (b'panda', __panda__), (b'llama', __llama__), (b'ocelot', __ocelot__), (b'ravager', __ravager__), (b'turtle', __turtle__), (b'phantom', __phantom__), (b'dolphin', __dolphin__), (b'fox', __fox__), (b'parrot', __parrot__), (b'rabbit', __rabbit__), (b'salmon', __salmon__), (b'spider', __spider__), (b'wraith', __wraith__), (b'zombie', __zombie__), (b'creeper', __creeper__)), ((b'piglin', __piglin__), (b'ghast', __ghast__), (b'shulker', __shulker__), (b'enderman', __enderman__), (b'villager', __villager__), (b'pillager', __pillager__), (b'guardian', __guardian__)), ((b'squid', __squid__),),)
        for seq in dual:
            for tag, fn in seq:
        ...
        for slot,row in enumerate(rows):
            raw=__vine__((slot,row));pulse.append((slot,len(raw),zlib.crc32(raw)&0xffffffff,zlib.adler32(raw)&0xffffffff,hashlib.blake2s(seed+raw,digest_size=16).hexdigest()))
        rows.append((b'pulse',len(pulse),__hist__(one[2]&255 for one in pulse),hashlib.sha512(__vine__(pulse)).hexdigest()))
...
def __pare__(tree):
    ...
    def __mcase__(n):
        ...
        return []
    ops={ast.Add:lambda a,b:a+b,ast.Sub:lambda a,b:a-b,ast.Mult:lambda a,b:a*b,ast.Div:lambda a,b:a/b,ast.FloorDiv:lambda a,b:a//b,ast.Mod:lambda a,b:a%b,ast.BitOr:lambda a,b:a|b,ast.BitXor:lambda a,b:a^b,ast.BitAnd:lambda a,b:a&b,ast.LShift:lambda a,b:a<<b if isinstance(b,int) and 0<=b<=64 else (__meow__ for __meow__ in ()).throw(ValueError),ast.RShift:lambda a,b:a>>b if isinstance(b,int) and 0<=b<=64 else (__meow__ for __meow__ in ()).throw(ValueError),ast.Pow:lambda a,b:a**b if not isinstance(b,int) or abs(b)<=32 else (__meow__ for __meow__ in ()).throw(ValueError)}
    ...
    def __go__(n):
        ...
        if isinstance(n, ast.Assert):
            if __pure__(n.test) and (n.msg is None or __pure__(n.msg)):
                return None
            n.test=__go__(n.test)
            if n.msg is not None: n.msg=__go__(n.msg)
            return n
        ...
        if isinstance(n,ast.BinOp):
            lo,l=__sim__(n.left);ro,r=__sim__(n.right);fn=ops.get(type(n.op));done,out=__ev__(fn,l,r) if lo and ro and fn else (False,None);return __mk__(out,n) if done else __alg__(n)
...
def __vein__(code):
        ...
        for node in ast.walk(root):
            if isinstance(node, ast.Name):
                ...
                for alias in node.names:
                    ...
                    seen.add(name); bind.add(name); mark.add(name)
                ...
                if isinstance(getattr(node, 'name', None), ast.Name):
                    seen.add(node.name.id); bind.add(node.name.id); made.add(node.name.id); mark.add(node.name.id)
                for tp in getattr(node, 'type_params', []) or []:
                    n = getattr(tp, 'name', None)
                    if isinstance(n, str):
                        seen.add(n); bind.add(n); para.add(n)
                    elif isinstance(n, ast.Name):
                        seen.add(n.id); bind.add(n.id); para.add(n.id)
                ...
                if isinstance(n, str):
                    seen.add(n); bind.add(n); para.add(n)
            if hasattr(node, 'type_params') and getattr(node, 'type_params'):
    ...
    def __owncall__(node):
        ...
        def __clsname__(base):
            if isinstance(base, ast.Name):
                return base.id
            return None
        def __iscls__(base):
            for one in clss:
                if base == one or (__token__(one) and base == __pick__(one)): return True
            return False
        if isinstance(node.func, ast.Name):
            return __iscls__(node.func.id)
        if isinstance(node.func, ast.Attribute):
            base = __clsname__(node.func.value)
            return base is not None and __iscls__(base)
        if isinstance(node.func, ast.Call) and isinstance(node.func.func, ast.Name) and node.func.func.id == 'getattr' and len(node.func.args) >= 1 and len(node.func.args) <= 2:
            base = __clsname__(node.func.args[0])
            return base is not None and __iscls__(base)
        return False
    ...
    def __patt__(node):
        ...
        if isinstance(node, ast.MatchSingleton):
            return node
        if isinstance(node, ast.MatchStar):
            if node.name and node.name != "_" and __token__(node.name):
        ...
        if isinstance(node, ast.MatchMapping):
            ...
            if node.rest and node.rest != "_" and __token__(node.rest):
    ...
    def __lambda__(body, args, vals, tag, vararg=None, kwarg=None, keys=None):
        ...
        a, b, c = __vault__(tag)
        raw.add(id(a)); raw.add(id(b)); raw.add(id(c))
        rows = [ast.arg(arg=one) for one in args]
        rows.extend((ast.arg(arg=left), ast.arg(arg=right)))
        lock = [ast.Constant(a), ast.Constant(b)]
        call = lock + list(vals) if (vararg or kwarg) and not args else list(vals) + lock
        body = ast.IfExp(test=__guard__(left, right, ast.Constant(c)), body=body, orelse=__crash__())
    ...
    def __gorge__():
        ...
        for __meow__ in range(__spark__(seed + b'gorgedepth' + tick[0].to_bytes(4, 'little'), 2, 5)):
    ...
    def __evoker__(body):
        ...
        tick[0]+=1
        ns=len(body)+1
        states=[__spark__(seed+b'evoker'+ix.to_bytes(4,'little')+tick[0].to_bytes(4,'little'),1000000,9999999) for ix in range(ns)]
        svar=__mint__(used,seed+b'evokervar',mint)
        init=ast.Assign(targets=[ast.Name(id=svar,ctx=ast.Store())],value=ast.Constant(states[0]))
        checks=[]
        for idx in range(len(body)):
            test=ast.Compare(left=ast.Name(id=svar,ctx=ast.Load()),ops=[ast.Eq()],comparators=[ast.Constant(states[idx])])
            nxt=ast.Constant(states[idx+1])
            bodychunk=[body[idx],ast.Assign(targets=[ast.Name(id=svar,ctx=ast.Store())],value=nxt)]
            checks.append(ast.If(test=test,body=bodychunk,orelse=[]))
        lastdrop=checks[-1]
        lastdrop.body[-1]=ast.Break()
        return[init,ast.While(test=ast.Constant(True),body=checks,orelse=[])]
    def __tomek__(body):
        if len(body)<2 or len(body)>5:
            return body
        for node in body:
            if isinstance(node,(ast.Global,ast.Nonlocal,ast.Import,ast.ImportFrom,ast.FunctionDef,ast.AsyncFunctionDef,ast.ClassDef,ast.Break,ast.Continue)):
                return body
        tick[0]+=1
        cobble=len(body)
        lava=[__spark__(seed+b'tomek'+ix.to_bytes(4,'little')+tick[0].to_bytes(4,'little'),1000000,9999999) for ix in range(cobble+1)]
        ember=__mint__(used,seed+b'tomekvar',mint)
        soul=__mint__(used,seed+b'tomekdict',mint)
        silk=__mint__(used,seed+b'tomekidx',mint)
        chain=__mint__(used,seed+b'tomeknxt',mint)
        dust=[ast.Constant(lava[idx]) for idx in range(cobble)]
        torch=[ast.Tuple(elts=[ast.Constant(idx),ast.Constant(lava[idx+1])],ctx=ast.Load()) for idx in range(cobble)]
        pick=ast.Assign(targets=[ast.Name(id=soul,ctx=ast.Store())],value=ast.Dict(keys=dust,values=torch))
        ore=ast.Assign(targets=[ast.Name(id=ember,ctx=ast.Store())],value=ast.Constant(lava[0]))
        iron=ast.Assign(targets=[ast.Tuple(elts=[ast.Name(id=silk,ctx=ast.Store()),ast.Name(id=chain,ctx=ast.Store())],ctx=ast.Store())],value=ast.Call(func=ast.Attribute(value=ast.Name(id=soul,ctx=ast.Load()),attr='get',ctx=ast.Load()),args=[ast.Name(id=ember,ctx=ast.Load()),ast.Tuple(elts=[ast.Constant(0),ast.Constant(0)],ctx=ast.Load())],keywords=[]))
        gold=[]
        for idx in range(cobble):
            test=ast.Compare(left=ast.Name(id=silk,ctx=ast.Load()),ops=[ast.Eq()],comparators=[ast.Constant(idx)])
            bodychunk=[body[idx],ast.Assign(targets=[ast.Name(id=ember,ctx=ast.Store())],value=ast.Name(id=chain,ctx=ast.Load()))]
            gold.append(ast.If(test=test,body=bodychunk,orelse=[]))
        gold[-1].body[-1]=ast.Break()
        return[ore,pick,ast.While(test=ast.Constant(True),body=[iron]+gold,orelse=[])]
    def __helmet__(body):
        ...
        nether=[]
        for one in body:
            if not isinstance(one,(ast.Assign,ast.AugAssign,ast.Expr)):
                nether.append(one)
                continue
            slag=__mint__(used,seed+b'hls'+tick[0].to_bytes(4,'little'),mint)
            rock=__spark__(seed+b'hlv'+tick[0].to_bytes(4,'little'),10**4,10**8)
            coal=__mint__(used,seed+b'hlc'+tick[0].to_bytes(4,'little'),mint)
            ...
            nether.append(ast.Assign(targets=[ast.Name(id=glass,ctx=ast.Store())],value=ast.Constant(glassv)))
            guard=ast.Compare(left=ast.Name(id=coal,ctx=ast.Load()),ops=[ast.Eq()],comparators=[ast.Name(id=glass,ctx=ast.Load())])
            inner=ast.Try(body=[ast.If(test=guard,body=[ast.Pass()],orelse=[one])],handlers=[ast.ExceptHandler(type=ast.Name(id='ZeroDivisionError',ctx=ast.Load()),name=None,body=[ast.Pass()])],orelse=[],finalbody=[ast.Assign(targets=[ast.Name(id=slag,ctx=ast.Store())],value=ast.Constant(rock))])
            loop=ast.For(target=ast.Name(id=flint,ctx=ast.Store()),iter=ast.List(elts=[ast.Constant(None)],ctx=ast.Load()),body=[inner,ast.Break()],orelse=[])
            nether.append(loop)
            tick[0]+=1
        return nether
    def __smoke__(body):
        ...
        for one in body:
            if done < 1 and not isinstance(one, (ast.Global, ast.Nonlocal, ast.Break, ast.Continue, ast.Import, ast.ImportFrom, ast.Assign, ast.AnnAssign, ast.AugAssign, ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
    ...
    def __ember__(val):
        ...
        raw.add(id(num.left)); raw.add(id(num.right))
        return __lambda__(ast.Call(func=ast.Name(id=alts[(idx + seed[2]) % len(alts)], ctx=ast.Load()), args=[num], keywords=[]), [], [], b'ember')
    def __rift__(val):
        size = len(val)
        if size < 128:
            return [val]
    ...
    def __stray__(val):
        ...
        if val and len(val) <= 24 and ((len(val) + seed[17] + tick[0]) & 63) == 10:
            tick[0] += 1
            blaze = [__spark__(seed + b'xork' + ix.to_bytes(2, 'little') + tick[0].to_bytes(4, 'little'), 1, 65535) for ix in range(len(val))]
            creeper = [ord(ch) ^ kv for ch, kv in zip(val, blaze)]
            ender = ast.List(elts=[ast.Constant(ev) for ev in creeper], ctx=ast.Load())
            ghast = ast.List(elts=[ast.Constant(kr) for kr in blaze], ctx=ast.Load())
            piston = __mint__(used, seed + b'xors' + tick[0].to_bytes(4, 'little'), mint)
            wither = __mint__(used, seed + b'xorb' + tick[0].to_bytes(4, 'little'), mint)
            spider = ast.GeneratorExp(elt=ast.Call(func=ast.Name(id='chr', ctx=ast.Load()), args=[ast.BinOp(left=ast.Name(id=piston, ctx=ast.Load()), op=ast.BitXor(), right=ast.Name(id=wither, ctx=ast.Load()))], keywords=[]), generators=[ast.comprehension(target=ast.Tuple(elts=[ast.Name(id=piston, ctx=ast.Store()), ast.Name(id=wither, ctx=ast.Store())], ctx=ast.Store()), iter=ast.Call(func=ast.Name(id='zip', ctx=ast.Load()), args=[ender, ghast], keywords=[]), ifs=[], is_async=0)])
            return __carry__(ast.Call(func=ast.Call(func=ast.Name(id='getattr', ctx=ast.Load()), args=[ast.Constant(''), __ember__('join')], keywords=[]), args=[spider], keywords=[]), b'xorchar')
        ...
        if val and len(val) <= 96 and ((len(val) + seed[24] + tick[0]) & 31) == 2:
            out = ast.BinOp(left=__ember__('%s'), op=ast.Mod(), right=out)
        if val and len(val) <= 48 and ((len(val) + seed[29] + tick[0]) & 127) == 5:
        ...
        if val and len(val) <= 64 and ((len(val) + seed[12] + tick[0]) & 31) == 1:
    ...
    def __calc__(val):
        ...
        if ((abs(val) + seed[30 % len(seed)] + tick[0]) & 7) == 3:
            km = 7 + ((abs(val) + seed[31 % len(seed)] + tick[0]) % 256)
            expr = ast.BinOp(left=ast.BinOp(left=expr, op=ast.Mult(), right=ast.Constant(km)), op=ast.FloorDiv(), right=ast.Constant(km))
...
({bookf} is not {varsf}({biobox}) or {globf} is not {bookf}[{__alias__('globals')}]() or not {callf}({mapf}) or not {callf}({inputf}) or {typef}({listf}({mapf}({intf},{listf}()))).__name__!='list' or {typef}({bytef}()).__name__!='bytearray' or {strf}({boolf}(1))!='True' or {hexf}({wide + 65})!='A') and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
def {dawn}(v,o,p):
 rows={bytesf}.fromhex(p)
 {lenf}(rows)!={lenf}(v) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return {bytesf}(((ord(one)-o)^rows[slot]) for slot,one in enumerate(v))
def {dusk}(v):
 ({lenf}(v)&1) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
...
def {loom}(v,p):
 ...
 {lenf}(rows)!={lenf}(fog) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return {bytesf}(one^rows[slot] for slot,one in enumerate(fog))
def {mire}(v,o):
 rows=[ord(one) for one in v]
 if not rows:
  return (0,0,0)
 rows[0]<o and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return (len(rows),min(rows),max(rows))
def {sootf}(v):
 all(isinstance(one,int) for one in v) or (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return (len(v),sum(v)&0xffffffff)
def {brimf}(v):
 (len(v)&1) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return v[::2]
def {crustf}(blob):
...
def {pearlf}(v,a,n):
 ...
 len(veil)!=len(v) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return bytes(((ord(one)-a)^veil[slot]) for slot,one in enumerate(v))
def {mazef}(q,a,n,need):
 veil=bytes.fromhex(n)
 len(veil)!=need and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 len(q)!=(need*2) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return bytes(((ord(q[slot*2])-a)^veil[slot]) for slot in range(need))
def {cordf}(c,f):
 all(isinstance(one,int) for one in c) or (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
...
def {beadf}(q):
 ...
 row=q[1::2]
 for one in row:
  ord(one)<0x3041 and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return len(row)
...
def {knotf}(v,need):
 len(v)!=need and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return (v[:1],v[-1:],need)
def {amberf}(blob):
...
def {glazef}(blob,mark):
 mark[0]!=len(blob) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 len(mark[1])>8 and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 (sum(mark[1])&0xffffffff)!=mark[2] and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
 return blob
def {reef}(blob):
 glow=0
 slot=0
...
def {rune}(k,v,o,p,r,u,l,f,t,h,s,e,g,m,y,n,q,c,d,x):
  ...
  spec={cask}(r)
  spec[1]!=(rows[0]*4) and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
  ash={dawn}(v,o,p)
  ash={ashf}(ash,{dusk}(r))
  ash={ashf}(ash,{kiln}(u,l,f))
  ash={ashf}(ash,{loom}(t,p))
  fog={crustf}(ash)
  fog[0]!=rows[0] and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
  tips={flaref}(ash)
  tips[2]!=rows[0] and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
  {knotf}(y,rows[0])
  {beadf}(q)!=rows[0] and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
  pearl={pearlf}(y,a,n)
  {ashf}(pearl,ash)
  ...
  {combf}({lockf}(ash),x)
  mark={rayf}(q)
  mark[0]!=rows[0] and (__meow__ for __meow__ in ()).throw(RuntimeError('bad'))
  lace={silkf}(ash)
...
"""
    ...
    def __tidal__(node):
        if isinstance(node, ast.For):
            nvar = __mint__(used, seed + b'tidal', mint)
            used.add(nvar)
            dead = __spark__(seed + b'tidalv', 1, 0xffffffff)
            pre = ast.Assign(targets=[ast.Name(id=nvar, ctx=ast.Store())], value=ast.Constant(dead))
            ret = ast.Try(body=[node], handlers=[ast.ExceptHandler(type=None, name=None, body=[ast.Pass()])], orelse=[], finalbody=[])
            return [pre, ret]
        if isinstance(node, ast.While):
            if isinstance(node.test, ast.Constant) and node.test.value is True:
                node.test = ast.UnaryOp(op=ast.Not(), operand=ast.Constant(False))
            nvar = __mint__(used, seed + b'tidal', mint)
            used.add(nvar)
            dead = __spark__(seed + b'tidalv', 1, 0xffffffff)
            pre = ast.Assign(targets=[ast.Name(id=nvar, ctx=ast.Store())], value=ast.Constant(dead))
    ...
    def __brume__(val):
        ...
        n=2+((len(val)+tick[0])%2)
        parts=[]
        at=0
        for i in range(n):
            chunk=len(val)//n+(1 if i<(len(val)%n) else 0)
            parts.append(val[at:at+chunk])
            at+=chunk
        if len(parts)<2:
            return ast.Constant(val)
        tick[0]+=1
        if tick[0]&1:
            return ast.BinOp(left=ast.Constant(parts[0]),op=ast.Add(),right=__brume__(parts[1] if len(parts)==2 else ''.join(parts[1:])))
        return ast.Call(func=ast.Attribute(value=ast.Constant(''),attr='join',ctx=ast.Load()),args=[ast.List(elts=[ast.Constant(p) for p in parts],ctx=ast.Load())],keywords=[])
    def __shape__(node):
        if node is None:
            return None
        if isinstance(node, ast.Module):
        ...
        if hasattr(ast, 'TypeAlias') and isinstance(node, ast.TypeAlias):
            if isinstance(getattr(node, 'name', None), ast.Name) and __token__(node.name.id):
                node.name.id = __pick__(node.name.id)
            for tp in getattr(node, 'type_params', []) or []:
                n = getattr(tp, 'name', None)
                if isinstance(n, str) and __token__(n):
                    tp.name = __pick__(n)
        ...
        if isinstance(node, ast.FunctionDef):
            ...
            node = __cast__(node, 'shape', {'body', 'type_params'})
        ...
        if isinstance(node, ast.Subscript) and isinstance(node.ctx, ast.Load):
            sl = __gloom__(node.slice)
            if isinstance(sl, ast.Tuple):
                sl.elts = [__gloom__(e) if not isinstance(e, ast.Starred) else ast.Starred(value=__gloom__(e.value), ctx=e.ctx) for e in sl.elts]
            return ast.Call(func=ast.Call(func=ast.Name(id='getattr', ctx=ast.Load()), args=[__gloom__(node.value), __ember__('__getitem__')], keywords=[]), args=[sl], keywords=[])
        ...
        if isinstance(node, ast.Assign):
            ...
            if len(node.targets) > 1:
                ...
                if __crimson__:
                    __cattt__ = __gloom__(node.value)
                    __yepppp__ = __mint__(used, seed + b'multiassign' + tick[0].to_bytes(4, 'little'), mint); tick[0]+=1
                    __yep__ = ast.Assign(targets=[ast.Name(id=__yepppp__, ctx=ast.Store())], value=__cattt__)
                    __ash__=[__yep__]
                    for __meowww__ in node.targets:
                        if isinstance(__meowww__, ast.Attribute) and isinstance(__meowww__.ctx, ast.Store):
                            __gold__ = __pick__(__meowww__.attr) if isinstance(__meowww__.value, ast.Name) and __meowww__.value.id in own and __meowww__.attr in mark and __token__(__meowww__.attr) else __meowww__.attr
                            __ash__.append(ast.Expr(value=ast.Call(func=ast.Name(id='setattr', ctx=ast.Load()), args=[__meowww__.value, __ember__(__gold__), ast.Name(id=__yepppp__, ctx=ast.Load())], keywords=[])))
                        elif isinstance(__meowww__, ast.Subscript) and isinstance(__meowww__.ctx, ast.Store):
                            __ash__.append(ast.Expr(value=ast.Call(func=ast.Call(func=ast.Name(id='getattr', ctx=ast.Load()), args=[__meowww__.value, __ember__('__setitem__')], keywords=[]), args=[__meowww__.slice, ast.Name(id=__yepppp__, ctx=ast.Load())], keywords=[])))
                        else:
                            __ash__.append(ast.Assign(targets=[__meowww__], value=ast.Name(id=__yepppp__, ctx=ast.Load())))
                    return __ash__
            node.value = __gloom__(node.value)
            return node
        if isinstance(node, ast.AnnAssign):
            if node.value is not None and isinstance(node.target, ast.Name) and __owncall__(node.value): own.add(node.target.id)
            if node.annotation is not None:
                node.annotation = __gloom__(node.annotation)
        ...
        if isinstance(node, ast.For):
            ...
            if ((seed[tick[0] % len(seed)] + tick[0]) & 31) == 0:
                return __tidal__(node)
            return node
        if isinstance(node, ast.AsyncFor):
            node.iter = __gloom__(node.iter)
            return node
        ...
        if isinstance(node, (ast.ListComp, ast.SetComp, ast.GeneratorExp)):
            __netherite__=set()
            for __gold__ in node.generators:
                for __ash__ in ast.walk(__gold__.target):
                    if isinstance(__ash__, ast.Name) and isinstance(__ash__.ctx, ast.Store):
                        __netherite__.add(__ash__.id)
            __yeppp__=set()
            for __gold__ in node.generators:
                def __cattt__(__yep__):
                    if isinstance(__yep__, ast.Name) and isinstance(__yep__.ctx, ast.Store) and __token__(__yep__.id):
                        __crimson__=__pick__(__yep__.id); __yep__.id=__crimson__; __yeppp__.add(__crimson__)
                    elif isinstance(__yep__, (ast.Tuple, ast.List)):
                        for __diamond__ in __yep__.elts: __cattt__(__diamond__)
                    elif isinstance(__yep__, ast.Starred): __cattt__(__yep__.value)
                __cattt__(__gold__.target)
            __yeppppp__=__yeppp__.union(__netherite__); lock.append(__yeppppp__); __deepslate__.append(set())
            node.elt = __gloom__(node.elt)
            for __gold__ in node.generators:
                __gold__.iter = __gloom__(__gold__.iter)
                __gold__.ifs = [__gloom__(one) for one in __gold__.ifs]
            lock.pop(); __deepslate__.pop()
            return node
        if isinstance(node, ast.DictComp):
            __netherite__=set()
            for __gold__ in node.generators:
        ...
        if isinstance(node, ast.While):
            node.test = __gloom__(node.test)
            if ((seed[tick[0] % len(seed)] + tick[0]) & 31) == 3:
                return __tidal__(node)
            return node
        if isinstance(node, ast.IfExp):
            node.test = __gloom__(node.test)
    ...
    def __perma__(root):
        ...
        for i,node in enumerate(root.body):
            ...
            if node.args.vararg or node.args.kwarg: continue
            if len(node.args.args)<1 or len(node.args.args)>2: continue
            if node.args.defaults or node.args.kw_defaults: continue
            if len(node.body)!=1: continue
            if not isinstance(node.body[0],ast.Return) or node.body[0].value is None: continue
            funcs[node.name]=(node,i)
        for name,(fdef,idx) in list(funcs.items()):
            calls=[]
...
def {hand}():
 ...
 if {screef} and {screef}!='0':
  __import__({__alias__('os')})._exit(1)
 {cragf}={shalex}.environ.get('LD_PRELOAD')
 if {cragf}:
  __import__({__alias__('os')})._exit(1)
 try:
  {cragf}=__import__({__alias__('ctypes')})
  {cragf}.CDLL('libc.so.6').prctl(4,0)
 except:pass
 if __import__({__alias__('threading')}).active_count()>1:
  __import__({__alias__('os')})._exit(1)
 {screef}=0
 try:
  for {cragf} in sys._current_frames().values():
   while {cragf} is not None:
    if {cragf}.f_trace is not None:
     {screef}=1
    {cragf}={cragf}.f_back
 except:pass
 if {screef}:
 ...
 {drusef}=''
 try:
  {cragf}=open('/proc/self/maps')
  {drusef}={cragf}.read()
  {cragf}.close()
 except:pass
 if {drusef} and 'frida' in {drusef}.lower():
  __import__({__alias__('os')})._exit(1)
 if globals().get('__name__')!='__main__':
  __import__({__alias__('os')})._exit(1)
 {fenf}=0
 try:
  for {screef} in range(6):
   if sys.monitoring.get_tool({screef}):
    {fenf}=1
 except:pass
 if {fenf}:
  __import__({__alias__('os')})._exit(1)
...
def {cliff}(code):
 ...
 while hold:
  ...
  for item in one.co_consts:
   if isinstance(item,kind): hold.append(item)
 if not rows: __import__({__alias__('os')})._exit(1)
 return rows
def {frost}(code):
 rows=[]
...
def {scan}(code):
 ...
 return row
def {huskf}(code):
 {scan}(code)!=({rift},{cull},{thorn},{flake},{peat}) and __import__({__alias__('os')})._exit(1)
 return code
def {grovef}(ct,name,restype,argtypes):
 if getattr(ct,'__name__','')!='ctypes' or getattr(getattr(ct,'_CFuncPtr',None),'__module__','')!='_ctypes': __import__({__alias__('os')})._exit(1)
 hold=getattr(ct.pythonapi,name,None)
 if hold is None: __import__({__alias__('os')})._exit(1)
 if not isinstance(hold,ct._CFuncPtr): __import__({__alias__('os')})._exit(1)
...
def {split}(blob,exp):
 ...
 (zlib.adler32(blob[:top])&0xffffffff)^(zlib.crc32(blob[:top])&0xffffffff)!=exp and __import__({__alias__('os')})._exit(1)
 return exp
...
def {gully}(blob,exp):
 ...
 rows[2]!=exp[2] and __import__({__alias__('os')})._exit(1)
 return rows
def {beryl}(blob,exp):
...
def {shalef}(blob,exp):
 ...
 rows[1]!=exp[1] and __import__({__alias__('os')})._exit(1)
 rows[2]!=exp[2] and __import__({__alias__('os')})._exit(1)
 rows[3]!=exp[3] and __import__({__alias__('os')})._exit(1)
 rows[4]!=exp[4] and __import__({__alias__('os')})._exit(1)
 return rows
def {shardy}(blob):
 ...
 hashlib.sha256(tab+inv+salt+{vinef}((add,step,twist,turn,drift,mask))).hexdigest()!=sig and __import__({__alias__('os')})._exit(1)
...
def {stampf}(blob):
 ...
 hashlib.sha256(bytes.fromhex({ore!r})+shell+{skin}.encode()+{storm}.encode()+{seal}.to_bytes(4,'little')).hexdigest()!={mesh!r} and __import__({__alias__('os')})._exit(1)
 {drusef}(shell)!={chaffk!r} and __import__({__alias__('os')})._exit(1)
...
def {runf}():
 ...
 {coref}={openf}()
 {glowf}({coref})!={tuffk} and __import__({__alias__('os')})._exit(1)
 {quill}.get('exec') or __import__({__alias__('os')})._exit(1)
 {huskf}({coref})
 try:
  return {evalf}({coref})
 finally:
...
"""
    ...
    sfprint=(len(ore)^(zlib.crc32(ore)&0xffff))&0xffff
    pack = __gasket__(ore)
    ...
    __coreb__(core, leftk, rightk, mistk, dustk, cloakk, lanek, spurk) != pack and (__meow__ for __meow__ in ()).throw(ValueError('core'))
...
def __concac__():
 try:
  ...
  if getattr(ctypes,'__name__','')!='ctypes' or getattr(getattr(ctypes,'_CFuncPtr',None),'__module__','')!='_ctypes':return __ditmemay__()
...
def __uwu__():
  ...
  try:inspect.getsource=lambda *a,**k:(__meow__ for __meow__ in ()).throw(OSError);inspect.getsourcelines=lambda *a,**k:(__meow__ for __meow__ in ()).throw(OSError);inspect.findsource=lambda *a,**k:(__meow__ for __meow__ in ()).throw(OSError);inspect.getfile=lambda *a,**k:(__meow__ for __meow__ in ()).throw(OSError);inspect.getmodule=lambda *a,**k:None;inspect.currentframe=lambda *a,**k:None;inspect.stack=lambda *a,**k:[]
  except:pass
...
def __checkvar__():
 ...
 return 0
def __owo__():
 for one in (lambda:sys.gettrace() is None,lambda:sys.getprofile() is None,lambda:getattr(threading,'_trace_hook',None) is None,lambda:getattr(threading,'_profile_hook',None) is None,lambda:not getattr(sys.flags,'inspect',0),lambda:not getattr(sys.flags,'debug',0),lambda:sys.breakpointhook is sys.__breakpointhook__,lambda:sys.excepthook is sys.__excepthook__,lambda:sys.displayhook is sys.__displayhook__,lambda:type(open).__name__=='builtin_function_or_method',lambda:type(print).__name__=='builtin_function_or_method',lambda:__import__.__module__ in ('builtins',None)):
  try:
...
__depvcl__()
"""
    outer=outer.replace("(__meow__ for __meow__ in ()).throw(SystemExit)","__baoloi__()").replace("raise SystemExit","__baoloi__()").replace(" and 1/0"," and __baoloi__()").replace("__SFPRINT__",str(sfprint))
    ...
    __wrapb__(wrap, shellk, glassk, forgek, stampk) != pack and (__meow__ for __meow__ in ()).throw(ValueError('wrap'))
    ...
    ember=ember.replace(f"(tag!={seal} or", f"bad=(tag!={seal} or").replace("(bad or any(", "bad=(bad or any(", 1).replace("or hit) and 1/0;blob=bytes(", f"or hit);bad and 1/0;mix=((tag[0]^{seal}[0])^((tag[1]^{seal}[1])&255)^((tag[2]^{seal}[2])&255)^(97 if bad else 0));blob=bytes(")
    ...
    tags=('blob','built','drift','right','readn','runn','glow','tint','need','base','mask','left','tmp','hit','byte','way','read','box','code','run','row','one','slot','key','sys','os','ct','b','j','h','l','z','s','thr','flag','fail','tag','size','bad','mix','add','path','ok','item')
    ...
    return __head__(__cowl__(__sear__(__flux__("\n".join([crust, cave, ember]), seed), seed), seed), seed, stamp, flag)
def __sear__(text, seed):
    ...
    uni=''.join(one for one, __meow__ in __marble__(seed + b'searuni', 3)).encode('utf-8','surrogatepass');name = __sigil__(seed + b'searname' + uni, 11)
    ...
    coal = f"(lambda {name[4]}:(lambda {name[5]}:{name[5]})({name[4]}))((lambda {name[8]},{name[9]}:(int.from_bytes({name[8]},'little')^int.from_bytes(({name[9]}*((len({name[8]})>>8)+1))[:len({name[8]})],'little')).to_bytes(len({name[8]}),'little'))(getattr({__aim__()}({wood}),{void})({name[2]}),bytes((({name[0]}*{lead})&255)^{iron}^(({name[0]}*{gold})&255)^{zinc} for {name[0]} in range(256))))"
    ash = f"(lambda {name[6]}:(lambda {name[7]}:{name[7]})({name[6]}))((lambda {name[10]}:(getattr({__aim__()}({wind}),{metal}),getattr({__aim__()}({ice}),{metal}),getattr({__aim__()}({rock}),{metal}))[{name[10]}[0]]({name[10]}[1:]))({coal}))"

    inner = (
        f"(lambda {name[2]}:"
        f"(lambda {name[3]}:("
        f"(getattr({__aim__()}({water}),{fire})({name[3]})%(1<<32))!={crc} "
        f"and 1/0,"
        f"getattr({__aim__()}({earth}),{air})(getattr({__aim__()}({moon}),{star})({name[3]}),getattr({__aim__()}({earth}),{sky})())"
        f")[-1])"
        f"({ash})"
        f")({melt})"
    )
...
def __cloak__(src, seed):
   ...
   name = __sigil__(seed + b'veilname', 32);tag = ['__yepppppp__', '__meoooo__', '__deptrai__'];tag.sort(key=lambda one:hashlib.sha256(seed + b'veilanchor' + one.encode()).digest());name[:3] = tag
   wkey = __spark__(seed + b'veilword', 1, 255);step = __spark__(seed + b'veilstep', 1, 251)
   words = ('base64', 'b85decode', 'zlib', 'bz2', 'lzma', 'decompress', 'ctypes', 'pythonapi', 'PyMarshal_ReadObjectFromString', 'PyEval_EvalCode', 'create_string_buffer', 'cast', 'c_char_p', 'c_long', 'py_object', '_CFuncPtr', 'hashlib', 'sha256', 'hexdigest', 'globals', 'builtins', 'builtin_function_or_method', 'open', 'read', 'rb', '__file__', 'replace', 'sys', 'modules', 'pop', '_ctypes', 'CFuncPtr')
   msgsrc = __show__(*__hide__("\033[95m>> Dung co deobf em yeu a.\033[0m", seed + b'veilmsg'), name[24])
   ...
   shotsrc = __show__(*__hide__(shot, seed + b'veilshot'), name[21])
   ...
   inner = inner.replace(f"{name[6]}={blob};{name[7]}=", f"{name[7]}=").replace(f"getattr({name[8]},{b85})({name[6]})", f"getattr({name[8]},{b85})({blob})")
   inner = inner.replace(f",{name[19]}().pop({name[16]!r},None),{name[18]}(", f",{name[19]}().__setitem__({name[9]!r},None),{name[19]}().__setitem__({name[6]!r},''),{name[19]}().__setitem__({name[7]!r},''),{name[19]}().__setitem__({name[8]!r},None),{name[19]}().__setitem__({name[13]!r},None),{name[19]}().__setitem__({name[14]!r},None),{name[19]}().__setitem__({name[15]!r},None),{name[19]}().__setitem__({name[17]!r},None),{name[19]}().__setitem__({name[24]!r},None),{name[19]}().__setitem__({name[27]!r},None),{name[19]}().__setitem__({name[28]!r},''),{name[19]}().__setitem__({name[30]!r},0),{name[19]}().pop({name[16]!r},None),{name[18]}(")
   return inner
def __ward__(seed):
   return f"{__totem__}=__import__(" + __spell__('builtins', seed + b'obsidianward') + ").__dict__;" + f"{__totem__}={__totem__}.get('dict')({__totem__});"
def __warp__(text, seed, mark):
   __end__=__builtins__ if isinstance(__builtins__,dict) else vars(__builtins__)
   __mob__=frozenset(one for one in __end__ if not (one.startswith('__') and one.endswith('__')))
   __xp__=[0]
   class __ender__(ast.NodeTransformer):
      def visit_Attribute(self, node):
         node.value=self.visit(node.value)
         if isinstance(node.ctx,ast.Load):
            __axe__=ast.Name(id='getattr',ctx=ast.Load())
            __axe__=self.visit(__axe__)
            __iron__=ast.Constant(node.attr)
            __iron__=self.visit(__iron__)
            return ast.Call(func=__axe__,args=[node.value,__iron__],keywords=[])
         return node
      def visit_Name(self, node):
         if node.id in __mob__:
            __xp__[0]+=1
            __gem__=__spell__(node.id,seed+b'obsidianwarp'+__xp__[0].to_bytes(4,'little')+b'name')
            return ast.Subscript(value=ast.Name(id=__totem__,ctx=ast.Load()),slice=ast.parse(__gem__,mode='eval').body,ctx=ast.Load())
         return node
      def visit_Constant(self, node):
         __sword__=node.value
         if isinstance(__sword__,str) and __sword__!=mark:
            if 1<len(__sword__)<=95 and any('a'<=__charm__<='z' or 'A'<=__charm__<='Z' for __charm__ in __sword__):
               __xp__[0]+=1
               __sand__=(len(__sword__)+1)//2
               __coal__=__spell__(__sword__[:__sand__],seed+b'obsidianwarp'+__xp__[0].to_bytes(4,'little')+b'strl')
               __water__=__spell__(__sword__[__sand__:],seed+b'obsidianwarp'+__xp__[0].to_bytes(4,'little')+b'strr')
               __gem__='('+__coal__+'+'+__water__+')'
               return ast.parse(__gem__,mode='eval').body
            if len(__sword__)==0:return node
            __xp__[0]+=1
            __bits__=bytes(__sword__,'utf-8')
            if not __bits__:
               __gem__='bytes(()).decode()'
            elif len(__sword__)>95:
               return ast.parse(repr(__sword__),mode='eval').body
            else:
               __gem__='bytes(('+','.join(map(str,__bits__))+(',' if len(__bits__)==1 else '')+')).decode()'
            return self.visit(ast.parse(__gem__,mode='eval').body)
         if isinstance(__sword__,int) and not isinstance(__sword__,bool) and (__sword__>255 or __sword__<-255):
            __xp__[0]+=1
            __gem__=__smelter__(__sword__,seed+b'obsidianwarp'+__xp__[0].to_bytes(4,'little')+b'int')
            return ast.parse(__gem__,mode='eval').body
         return node
   return ast.unparse(__ender__().visit(ast.parse(text)))
def __smooth__(text, seed):
   __sack__=0;__pist__='';__shard__=[]
   for __seed__ in tokenize.generate_tokens(io.StringIO(text).readline):
      __slime__=__seed__.type
      if __slime__ in (tokenize.ENCODING,tokenize.INDENT,tokenize.DEDENT):continue
      if __slime__ in (tokenize.NEWLINE,tokenize.NL):
         __shard__.append(' ' if (__sack__>0 or __pist__==':') else ';')
         continue
      __sprout__=__seed__.string
      if __pist__ and __sprout__ and (__pist__[-1].isalnum() or __pist__[-1]=='_') and (__sprout__[0].isalnum() or __sprout__[0]=='_'):
         __shard__.append(' ')
      __shard__.append(__sprout__)
      if __slime__==tokenize.OP and __sprout__ in '([{':__sack__+=1
      if __slime__==tokenize.OP and __sprout__ in ')]}':__sack__=max(0,__sack__-1)
      __pist__=__sprout__
   text=''.join(__shard__)
...
def __flux__(code, seed):
   ...
   words = ('base64', 'bz2', 'hashlib', 'lzma', 'marshal', 'zlib', 'b85decode', 'sha256', 'decompress', 'loads', 'exec', 'range', 'builtins', 'bytes', 'enumerate', 'type', 'hexdigest', 'ascii', 'len', 'int', 'from_bytes', 'to_bytes', 'little', 'ctypes', 'pythonapi', 'PyMarshal_ReadObjectFromString', 'c_char_p', 'c_long', 'py_object', 'create_string_buffer', 'cast', '__module__', 'builtin_function_or_method', 'sys', 'addaudithook', 'meta_path', '__name__', '_ctypes', '_CFuncPtr', 'modules', 'pop', 'CFuncPtr')
   words = tuple(tuple(ord(char) ^ wkey for char in word) for word in words)
   uni=''.join(one for one, __meow__ in __marble__(seed + b'fluxuni', 3)).encode('utf-8','surrogatepass');names = __sigil__(seed + b'fluxname' + uni, 26)
...
def __cowl__(code, seed):
   ...
   uni=''.join(one for one, __meow__ in __marble__(seed + b'cowluni', 3)).encode('utf-8','surrogatepass');name = __sigil__(seed + b'maskname' + uni, 46);tag = ['__yepppppp__', '__meoooo__', '__deptrai__'];tag.sort(key=lambda one:hashlib.sha256(seed + b'cowlanchor' + one.encode()).digest());name[:3] = tag
   ...
   gate = __spell__('builtins', seed + b'cowlgate')
   bit = __sigil__(seed + b'cowlflag', 1)[0]
   trap = (name[1] + '(' + name[2] + '[6])+chr(46)+' + name[1] + '(' + name[2] + '[34])', "'sys'+chr(46)+'settrace'", "'sys'+chr(46)+'setprofile'")
   body = f"{name[0]}={__aim__()};{name[1]}=lambda {name[15]}:''.join(chr({name[16]}^{key}) for {name[16]} in {name[15]});{name[41]}=lambda:(({name[0]}({gate}).__dict__.get({bit!r},0) or print('\\033[95m>> Dung co deobf em yeu a.\\033[0m',flush=True) or setattr({name[0]}({gate}),{bit!r},1)),(__meow__ for __meow__ in ()).throw(SystemExit(1)))[-1];{name[2]}={words!r};{name[3]}={blob};{name[4]}={shotsrc};{name[8]}={name[0]}({built}).__dict__[{get}];({name[0]}.__class__.__name__!='builtin_function_or_method' or {name[8]}({name[0]},{name[1]}({name[2]}[17]),'')!='builtins') and 1/0;({name[8]}.__class__.__name__!='builtin_function_or_method' or {name[8]}({name[8]},{name[1]}({name[2]}[17]),'')!='builtins') and 1/0;{name[17]}={name[0]}({name[1]}({name[2]}[14]));{name[18]}={name[8]}({name[17]},{name[1]}({name[2]}[15]));{name[6]}={name[8]}({name[17]},{__scatter__('addaudithook', seed + b'cowladd')});({name[6]}.__class__.__name__!='builtin_function_or_method' or {name[8]}({name[6]},{name[1]}({name[2]}[17]),'')!='sys') and 1/0;{name[7]}={name[8]}({name[17]},{__scatter__('meta_path', seed + b'cowlmeta')},());{name[9]}=(('_distutils_hack','DistutilsMetaFinder'),('_frozen_importlib','BuiltinImporter'),('_frozen_importlib','FrozenImporter'),('_frozen_importlib_external','PathFinder'));({name[7]} is None or any((({name[8]}({name[10]},{name[1]}({name[2]}[17]),{name[8]}(type({name[10]}),{name[1]}({name[2]}[17]),'')),{name[8]}({name[10]},{name[1]}({name[2]}[18]),type({name[10]}).__name__)) not in {name[9]} for {name[10]} in {name[7]}))) and 1/0;{name[45]}=({','.join(trap)});{name[6]}(lambda {name[43]},{name[44]},{name[42]}={name[45]}:{name[43]} in {name[42]} and {name[41]}());[{name[18]}.pop({name[1]}({name[2]}[{name[19]}]),None) for {name[19]} in (0,2,3,4,6,8,19,20)];{name[20]}={name[0]}({name[1]}({name[2]}[0]));{name[21]}={name[0]}({name[1]}({name[2]}[2]));{name[22]}={name[0]}({name[1]}({name[2]}[3]));{name[23]}={name[0]}({name[1]}({name[2]}[4]));{name[24]}={name[0]}({name[1]}({name[2]}[6]));{name[25]}={name[0]}({name[1]}({name[2]}[8]));{name[26]}={name[0]}({name[1]}({name[2]}[10]));{name[34]}={name[0]}({name[1]}({name[2]}[19]));({name[8]}({name[34]},{name[1]}({name[2]}[18]),'')!={name[1]}({name[2]}[19]) or {name[8]}({name[8]}({name[34]},'_CFuncPtr'),{name[1]}({name[2]}[17]),'')!={name[1]}({name[2]}[20])) and 1/0;{name[27]}={name[8]}({name[20]},{name[1]}({name[2]}[1]));{name[28]}={name[8]}({name[21]},{name[1]}({name[2]}[5]));{name[29]}={name[8]}({name[22]},{name[1]}({name[2]}[5]));{name[30]}={name[8]}({name[23]},{name[1]}({name[2]}[5]));{name[31]}={name[8]}({name[24]},{name[1]}({name[2]}[7]));{name[32]}={name[8]}({name[26]},{name[1]}({name[2]}[11]));{name[33]}={name[8]}({name[26]},{name[1]}({name[2]}[12]))"
